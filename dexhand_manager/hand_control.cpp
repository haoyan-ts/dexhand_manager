...
#include "service_interfaces/srv/setwristangle.hpp"
#include "service_interfaces/srv/getangleact.hpp"
#include "service_interfaces/srv/getwristangleact.hpp"
...
#include "service_interfaces/srv/getwristangleset.hpp"
#include "service_interfaces/srv/getposact.hpp"
#include "service_interfaces/srv/getposset.hpp"
#include "service_interfaces/srv/getwristposact.hpp"
#include "service_interfaces/srv/getwristposset.hpp"
...
#include "service_interfaces/srv/getwristcurrentact.hpp"
#include "service_interfaces/srv/geterror.hpp"
#include "service_interfaces/srv/getwristerror.hpp"
#include "service_interfaces/srv/gettemp.hpp"
#include "service_interfaces/srv/getwristtemp.hpp"
...
public:
    ...
    Hand_control() : Node("Hand_control")
    {
        ...
        Setwristangle_Server = this->create_service<service_interfaces::srv::Setwristangle>("Setwristangle",
                                    std::bind(&Hand_control::setwristangle_callback,this,_1,_2),
                                    rmw_qos_profile_services_default,
                                    callback_group_setwristangle);
        ...
        Getwristangleact_Server = this->create_service<service_interfaces::srv::Getwristangleact>("Getwristangleact",
                                    std::bind(&Hand_control::getwristangleact_callback, this, _1, _2),
                                    rmw_qos_profile_services_default,
                                    callback_group_getwristangleact);
        ...
        Getwristangleset_Server = this->create_service<service_interfaces::srv::Getwristangleset>("Getwristangleset",
                                    std::bind(&Hand_control::getwristangleset_callback, this, _1, _2),
                                    rmw_qos_profile_services_default,
                                    callback_group_getwristangleset);                            
        ...
        Getwristposact_Server = this->create_service<service_interfaces::srv::Getwristposact>("Getwristposact",
                                    std::bind(&Hand_control::getwristposact_callback, this, _1, _2),
                                    rmw_qos_profile_services_default,
                                    callback_group_getwristposact);                            
        ...
        Getwristposset_Server = this->create_service<service_interfaces::srv::Getwristposset>("Getwristposset",
                                    std::bind(&Hand_control::getwristposset_callback, this, _1, _2),
                                    rmw_qos_profile_services_default,
                                    callback_group_getwristposset);                            
        ...
        Getwristcurrentact_Server = this->create_service<service_interfaces::srv::Getwristcurrentact>("Getwristcurrentact",
                                    std::bind(&Hand_control::getwristcurrentact_callback, this, _1, _2),
                                    rmw_qos_profile_services_default,
                                    callback_group_getwristcurrentact);                            
        ...
        Getwristerror_Server = this->create_service<service_interfaces::srv::Getwristerror>("Getwristerror",
                                    std::bind(&Hand_control::getwristerror_callback, this, _1, _2),
                                    rmw_qos_profile_services_default,
                                    callback_group_getwristerror);                            
        ...
        Getwristtemp_Server = this->create_service<service_interfaces::srv::Getwristtemp>("Getwristtemp",
                                    std::bind(&Hand_control::getwristtemp_callback, this, _1, _2),
                                    rmw_qos_profile_services_default,
                                    callback_group_getwristtemp);                            
    }
private:
    // 声明服务回调组
    ...
    rclcpp::CallbackGroup::SharedPtr callback_group_setwristangle;
    ...
    rclcpp::CallbackGroup::SharedPtr callback_group_getwristangleact;
    rclcpp::CallbackGroup::SharedPtr callback_group_getwristangleset;
    rclcpp::CallbackGroup::SharedPtr callback_group_getwristposact;
    rclcpp::CallbackGroup::SharedPtr callback_group_getwristposset;
    ...
    rclcpp::CallbackGroup::SharedPtr callback_group_getwristcurrentact;
    rclcpp::CallbackGroup::SharedPtr callback_group_getwristerror;
    rclcpp::CallbackGroup::SharedPtr callback_group_getwristtemp;
    // 声明服务端
    ...
    rclcpp::Service<service_interfaces::srv::Setwristangle>::SharedPtr Setwristangle_Server;
    ...
    rclcpp::Service<service_interfaces::srv::Getwristangleact>::SharedPtr Getwristangleact_Server;
    rclcpp::Service<service_interfaces::srv::Getwristangleset>::SharedPtr Getwristangleset_Server;
    rclcpp::Service<service_interfaces::srv::Getwristposact>::SharedPtr Getwristposact_Server;
    rclcpp::Service<service_interfaces::srv::Getwristposset>::SharedPtr Getwristposset_Server;
    ...
    rclcpp::Service<service_interfaces::srv::Getwristcurrentact>::SharedPtr Getwristcurrentact_Server;
    rclcpp::Service<service_interfaces::srv::Getwristerror>::SharedPtr Getwristerror_Server;
    rclcpp::Service<service_interfaces::srv::Getwristtemp>::SharedPtr Getwristtemp_Server;
    // 声明回调函数，当收到要请求时调用该函数
    ...
    void setwristangle_callback(const service_interfaces::srv::Setwristangle::Request::SharedPtr request,
                               const service_interfaces::srv::Setwristangle::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "set_wristangle")
        {
            // 打印指令类型
            RCLCPP_INFO(this->get_logger(), "收到%s的请求", request->status.c_str());
            // 传递数据到数组
            ...
            send_buffer[5] = 0x0E;
            send_buffer[6] = 0x04;
            send_buffer[7] = ((request->yaw-2550) & 0xFF);
            send_buffer[8] = (((request->yaw-2550) >> 8) & 0xFF);
            send_buffer[9] = ((request->pitch-2266) & 0xFF);
            send_buffer[10] = (((request->pitch-2266) >> 8) & 0xFF);
            for(int i = 2;i < 11;i++)
            {
                check_sum += send_buffer[i];
            }
            send_buffer[11] = check_sum;
            ros_ser.write(send_buffer,12);
            loop_rate.sleep();    //等待100ms接收数据
            int count = ros_ser.available(); // count读取到缓存区数据的字节数，不等于0说明缓存里面有数据可以读取
            if (count != 0)  //等待接收数据
            {   
                ...
                if(recv_buffer[7] == 0x01)
                {
                    response->wristangle_accepted = true;
                    printf("设置指令成功\n");
                }
                else
                {
                    response->wristangle_accepted = false;
                    printf("设置指令失败\n");
                }
            }
        }
    ...
    void getwristangleact_callback(const service_interfaces::srv::Getwristangleact::Request::SharedPtr request,
                               const service_interfaces::srv::Getwristangleact::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristangleact")
        {
            // 打印请求
            RCLCPP_INFO(this->get_logger(), "收到一个来自%s的指令", request->status.c_str());
            // 传递数据到数组
            ...
            {   
                std::vector<unsigned char> recv_buffer(count);//开辟数据缓冲区，串口read读出的内容是无符号char类型
                count = ros_ser.read(&recv_buffer[0], count); // 读出缓存区缓存的数据，返回值为读到的数据字节数
                if(recv_buffer[4] == 0x30)
                {
                    response->curwristyawact = (int16_t)((recv_buffer[7] & 0xFF) + ((recv_buffer[8]<<8) & 0xFF00));
                    response->curwristpitchact = (int16_t)((recv_buffer[9] & 0xFF) + ((recv_buffer[10]<<8) & 0xFF00));
                    RCLCPP_INFO(this->get_logger(), "手腕俯仰角度为:%d 手腕偏转角度为:%d", 
                    (response->curwristyawact),(response->curwristpitchact));
                }
                else
                {
                    printf("无法读取手腕实际角度值\n");
    ...
    void getwristangleset_callback(const service_interfaces::srv::Getwristangleset::Request::SharedPtr request,
                               const service_interfaces::srv::Getwristangleset::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristangleset")
        {
            // 打印请求
            RCLCPP_INFO(this->get_logger(), "收到一个来自%s的指令", request->status.c_str());
            // 传递数据到数组
            ...
            {   
                ...
                if(recv_buffer[4] == 0x30)
                {
                    response->curwristyawset = (int16_t)((recv_buffer[7] & 0xFF) + ((recv_buffer[8]<<8) & 0xFF00));
                    response->curwristpitchset = (int16_t)((recv_buffer[9] & 0xFF) + ((recv_buffer[10]<<8) & 0xFF00));
                    RCLCPP_INFO(this->get_logger(), "手腕设置俯仰角度为:%d 手腕设置偏转角度为:%d", 
                    (response->curwristyawset),(response->curwristpitchset));
                }
                else
                {
                    printf("无法读取手腕角度设置值\n");
    ...
    void getwristposact_callback(const service_interfaces::srv::Getwristposact::Request::SharedPtr request,
                               const service_interfaces::srv::Getwristposact::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristposact")
        {
            // 打印请求
            RCLCPP_INFO(this->get_logger(), "收到一个来自%s的指令", request->status.c_str());
            // 传递数据到数组
            ...
            {   
                ...
                if(recv_buffer[4] == 0x30)
                {
                    response->curwristposact[0] = (int16_t)((recv_buffer[7] & 0xFF) + ((recv_buffer[8]<<8) & 0xFF00));
                    response->curwristposact[1] = (int16_t)((recv_buffer[9] & 0xFF) + ((recv_buffer[10]<<8) & 0xFF00));
                    RCLCPP_INFO(this->get_logger(), "手腕驱动器实际位置值为:%d %d", 
                    response->curwristposact[0],response->curwristposact[1]);
                }
                else
                {
                    printf("无法读取手腕驱动器实际位置值\n");
                } 
            }
        }
        else
                               ...
                               const service_interfaces::srv::Getposset::Response::SharedPtr response)
    {
        ...
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristposset")
        {
            // 打印请求
            RCLCPP_INFO(this->get_logger(), "收到一个来自%s的指令", request->status.c_str());
            // 传递数据到数组
    ...
    void getwristposset_callback(const service_interfaces::srv::Getwristposset::Request::SharedPtr request,
                               const service_interfaces::srv::Getwristposset::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristposset")
        {
            ...
            {   
                ...
                if(recv_buffer[4] == 0x30)
                {
                    response->curwristposset[0] = (int16_t)((recv_buffer[7] & 0xFF) + ((recv_buffer[8]<<8) & 0xFF00));
                    response->curwristposset[1] = (int16_t)((recv_buffer[9] & 0xFF) + ((recv_buffer[10]<<8) & 0xFF00));
                    RCLCPP_INFO(this->get_logger(), "手腕驱动器设置位置值为:%d %d", 
                    response->curwristposset[0],response->curwristposset[1]);
                }
                else
                {
                    printf("无法读取手腕驱动器设置位置值\n");
                } 
            }
        }
        else
        ...
        if(request->status == "get_forceact")
        {
            ...
            {   
                ...
                if(recv_buffer[4] == 0x11)
                {
                    for(int i=0;i<6;i++)
                    { 
                        response->curforceact[i] = (recv_buffer[7+i*2] & 0xFF) + ((recv_buffer[8+i*2]<<8) & 0xFF00);
                    }
    ...
    void getwristcurrentact_callback(const service_interfaces::srv::Getwristcurrentact::Request::SharedPtr request,
                               const service_interfaces::srv::Getwristcurrentact::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristcurrentact")
        {
            // 打印请求
            RCLCPP_INFO(this->get_logger(), "收到一个来自%s的指令", request->status.c_str());
            // 传递数据到数组
            ...
            {   
                ...
                if(recv_buffer[4] == 0x30)
                {
                    for(int i=0;i<2;i++)
                    { 
                        response->curwristcurrent[i] = (recv_buffer[7+i*2] & 0xFF) + ((recv_buffer[8+i*2]<<8) & 0xFF00);
                    }
                    RCLCPP_INFO(this->get_logger(), "手腕实际电流为:%d %d", 
                    response->curwristcurrent[0],response->curwristcurrent[1]);
                }
                else
                {
                    printf("无法读取手腕实际电流值\n");
    ...
    void getwristerror_callback(const service_interfaces::srv::Getwristerror::Request::SharedPtr request,
                               const service_interfaces::srv::Getwristerror::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristerror")
        {
            // 打印请求
            RCLCPP_INFO(this->get_logger(), "收到一个来自%s的指令", request->status.c_str());
            // 传递数据到数组
            ...
            {   
                ...
                if(recv_buffer[4] == 0x30)
                {
                    for(int i=0;i<2;i++)
                    { 
                        response->wristerror[i] = (recv_buffer[7+i] & 0xFF);
                    }
                    RCLCPP_INFO(this->get_logger(), "手腕故障码为:%d %d", 
                    response->wristerror[0],response->wristerror[1]);
                }
                else
                {
                    printf("无法读取手腕故障码\n");
    ...
    void getwristtemp_callback(const service_interfaces::srv::Getwristtemp::Request::SharedPtr request,
                               const service_interfaces::srv::Getwristtemp::Response::SharedPtr response)
    {
        u_int8_t check_sum = 0;
        rclcpp::WallRate loop_rate(10.0);
        // 首先判断指令类型
        if(request->status == "get_wristtemp")
        {
            // 打印请求
            RCLCPP_INFO(this->get_logger(), "收到一个来自%s的指令", request->status.c_str());
            // 传递数据到数组
            ...
            {   
                ...
                if(recv_buffer[4] == 0x30)
                {
                    for(int i=0;i<2;i++)
                    { 
                        response->wristtemp[i] = (recv_buffer[7+i] & 0xFF);
                    }
                    RCLCPP_INFO(this->get_logger(), "手腕电缸温度为:%d %d", 
                    response->wristtemp[0],response->wristtemp[1]);
                }
                else
                {
                    printf("无法读取手腕温度值\n");
